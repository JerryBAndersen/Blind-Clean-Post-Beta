...
public class LiftingBody : MonoBehaviour {
    ...
    public SteeringMode steeringMode = SteeringMode.Wings;
    ...
    void FixedUpdate() {
                ...
                case SteeringMode.Wings:
                    {
                        ...
                        bodydirection = new Vector3(horizSteer * Mathf.Abs(horizSteer) * wingTurnStrength, bodydirection.y * 100f, bodydirection.z * 100f);
                        pointdir += glide * bodydirection;
                        break;
                    }
            }
        ...
        if ((Mathf.Abs(leftvel.y) > forcethresh || Mathf.Abs(rightvel.y) > forcethresh)) {
            ...
            if (leftvel.y < -forcethresh) {
                ...
                rigid.AddForceAtPosition(leftforce + transform.forward * leftforce.magnitude/2f, transform.position);
            }
            if (rightvel.y < -forcethresh) {
                ...
                rigid.AddForceAtPosition(rightforce + transform.forward * rightforce.magnitude / 2f, transform.position);
            }
        }
        ...
        rigid.AddTorque(Mathf.Min(rigid.velocity.magnitude * rigid.velocity.magnitude, 1f) *    glide * towardsVel * Vector3.Cross(rigid.transform.forward, lowspeedlimit * 100f * rigid.velocity.normalized * .5f + .5f * pointdir.normalized));
        rigid.AddTorque(Mathf.Min(rigid.velocity.magnitude * rigid.velocity.magnitude, 1f)    *    towardsVel * Vector3.Cross(rigid.transform.up, 50f * Vector3.up));
